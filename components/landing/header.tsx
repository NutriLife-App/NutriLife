    ...
    >
      ...
      {(["ua", "en"] as const).map((lang) => (
        <button
          ...
          className={`relative z-10 flex-1 cursor-pointer rounded-full ${sizeStyles.labelSize} font-semibold tracking-wider transition-colors duration-200 select-none ${language === lang ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
        >
          {lang.toUpperCase()}
        </button>
      ))}
